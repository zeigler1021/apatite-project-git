taylor.uncertainty <- function (df, param) {
  ...
  if (param == "ft") {
    ...
    slope <- summary(lm(df$s.ft ~ df$db.ft))$coefficients[1,1]
  ...
  if (param == "volume") {
    slope <- summary(lm(df$s.v ~ df$db.v))$coefficients[1,1]
  ...
  if (param == "esr") {
    slope <- summary(lm(df$s.esr.ft ~ df$db.esr.ft))$coefficients[1,1]
