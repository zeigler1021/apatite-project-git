...
ggplot(aes(x = s.ft, y = db.ft, color= gc)) + #, key = sample, key2= broken, key3 = notes
  ...
  labs(x= "2D Ft", y= "3D Ft") + #title = "Manual 2D vs. Nano-CT 3D Ft"
  scale_color_tableau(palette = "Color Blind") +
  #scale_color_manual(values = cb5) +
...
lm(s.ft ~ 0 + db.ft, ellip)
1/1.083
lm(db.ft ~ 0 + s.ft, ellip)
```
...
ggplot(aes(x = s.v, y = db.v, color=gc)) + # 
  ...
  labs(title = "Manual 2D vs. Nano-CT 3D Volume", subtitle = "The slopes for A and B grains overlap, while C grains are distinct", x= "2D Volume (µm3)", y= "3D Volume (µm3)") +
...
p.ft + p.rs +plot_annotation(title = "Manual 2D vs. Nano-CT 3D Ft and Rs-Ft", subtitle = "The slopes for A and B grains overlap, while C grains are distinct")
```
...
p.ft.res <- ggplot(uncert_result_hex_ft, aes(j.w1, percent_diff)) +
  ...
  labs(x = "Maximum Width (µm)", y = "Ft Residuals as a % Difference") #, title = "Ft Residuals vs. Maximum Width", subtitle = latex2exp::TeX("The sd of Ft residuals are controlled by $\\textbf{size}.")
...
p.rs.res <- ggplot(uncert_result_hex_esr, aes(j.w1, percent_diff)) +
  geom_hline(mapping=aes(yintercept = 0)) +
  geom_point(mapping = aes(color = size.cat)) +
  scale_color_jco(palette = "default", 
                  name = "Legend", 
                  breaks = c("rare- small", "common", "rare- large"), 
                  labels = c("Small", "Average", "Large")) +
  labs(x = "Maximum Width (µm)", y = "Rs-Ft Residuals as a % Difference") #, title = "Rs-Ft Residuals vs. Maximum Width", subtitle = latex2exp::TeX("The sd of Rs-Ft residuals are controlled by $\\textbf{size}.")

p.ft.res + p.rs.res + plot_annotation(title = "Ft and Rs-Ft Residuals vs. Maximum Width", subtitle = latex2exp::TeX("The sd of Ft and Rs-Ft residuals are controlled by $\\textbf{size}."))
