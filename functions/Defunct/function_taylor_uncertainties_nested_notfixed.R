taylor.uncertainty.not.fixed <- function (df, param) {
  ...
  if (param == "ft") {
    ...
    sigma.y <- sqrt(1/(nrow(df)-2)*sum((df$s.ft - intercept - df$db.ft * slope)^2))
  ...
  if (param == "volume") {
    ...
    sigma.y <- sqrt(1/(nrow(df)-2)*sum((df$s.v - intercept - df$db.v * slope)^2))
  ...
  if (param == "esr") {
    ...
    sigma.y <<- sqrt(1/(nrow(df)-2)*sum((df$s.esr.ft - intercept - df$db.esr.ft * slope)^2))
